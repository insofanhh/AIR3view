...
@pytest.mark.parametrize('mode',['overlay','insert'])
def test_real_voice_mix_across_part_boundary(tmp_path,monkeypatch,mode):
    ...
    p['settings'].update(output_mode=None, narration_mode=mode,part_durations=[2],title='Kiểm thử trộn giọng')
    ...
    if mode == 'overlay':
        # Source is a 220 Hz tone, narration is 880 Hz. Prove source is muted,
        # not merely quieter, while the moving test pattern continues playing.
        import av
        import numpy as np
        ...
        # The voice crosses the split. The second part must stay muted at its
        # beginning, then restore source sound when the narration ends.
        ...
        with av.open(str(store.asset(p['id'], second['file']))) as container:
            samples = np.concatenate([f.to_ndarray()[0] for f in container.decode(audio=0)])
        assert amplitude(220, .2, .8) < amplitude(220, 1.2, 1.8) * .02
